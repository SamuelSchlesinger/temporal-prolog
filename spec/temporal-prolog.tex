...
these two signatures.  The first $k$ relation positions are ground inputs; a
successful call grounds the final output.  External predicates have the fixed
signatures listed above and MAY occur only as conditions; they MUST NOT occur
in rule results or be inserted into a world as external input.  The arithmetic symbols $+$, $-$,
...
\begin{enumerate}
  ...
  MAY be queried only at relational arity with $k$ ground inputs; the final
  output MAY contain variables.  Implementations MUST validate this profile
  before any query.
  \item Every variable in a negated condition, forward-chained head, or
  mode input is grounded by positive body conditions before observation.  Mode
  inputs are the first $k$ pattern positions, the \verb|is| expression, and
  comparison operands.  A pattern call binds only its output; clauses assume
  head inputs ground and MUST ground it.  Evaluators MUST defer mode-blocked
  positive conjuncts: source order MUST NOT cause failure, and a body with no
  safe order MUST be rejected.
  ...
  \item The general candidate base is the fixed-point closure of relaxed
  positive derivation plus every grounded current-world negative internal atom
  whose remaining body holds in that closure.  This preserves unsupported
  blockers admitted by the paper's classical models.
  \item General negative SCCs require a candidate base within the configured
  enumeration limit; excess is a resource error, not a semantic result.
...
\begin{enumerate}
  \item Both implementations accept every shared positive and reject every
  negative case, covering lexical and signature violations, temporal and
  arithmetic boundaries, unsafe modes, and query validation.  A positive case
  puts mode calls before grounding conjuncts; public queries cover external
  bindings and whole-program profile validation.
