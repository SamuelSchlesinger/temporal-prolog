...
\begin{enumerate}
  ...
  output MAY contain variables.
  \item Every variable in a negated condition, forward-chained head, or
  pattern-function input position is grounded by positive body conditions,
  $\at(X)$, unification with a ground term, or ground arithmetic before it is
  observed.  A pattern-function call binds only its final output after its
  inputs are ground.  Its clauses begin with head inputs ground and MUST
  ground the head output from those inputs and positive body conditions.
...
\begin{enumerate}
  ...
  reject every shared negative case.  Rejections cover arity conflicts,
  non-ASCII identifiers, malformed pattern-function or external signatures,
  oversized \verb|for| counts, and external results or assertions.  Positive
  cases cover contextual keyword namespaces.  Runtime cases reject signature
  changes across source, input, and history; internal-name injection;
  term-level previous; malformed arithmetic; unsafe pattern-function modes;
  and query-mode violations.  Public queries also test every external
  predicate family's bindings.
