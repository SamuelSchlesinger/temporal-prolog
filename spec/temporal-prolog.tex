...
literals may have a leading minus sign.  A comment begins with \verb|%| and
extends to the end of the line.  Whitespace and comments separate tokens.
...
The external predicates include $=$, $\true$, $\false$, and $\at/1$.
...
identified by term position.  A program MUST use each name consistently.
...
than \verb|since|, \verb|after|, and \verb|for|.  Implication binds least
tightly.  Parentheses SHOULD be used when a result combines conjunction with
\verb|until| or \verb|atnext|.
...
The portable executable profile defines integer arithmetic terms
$+$, $-$, $*$, \verb|div|, and \verb|mod|.  The predicate
$X\ \mathsf{is}\ E$ evaluates a ground expression $E$ and unifies $X$ with
its integer result.  Comparisons require ground integer operands.  Division
by zero fails without binding.  \verb|!=| and \verb|\=| abbreviate negated
Herbrand equality.
...
\begin{enumerate}
  ...
  reject every shared negative case.
  ...
  fresh identifiers do not collide with source identifiers.
  ...
  functions, unsupported classical blockers, and Section 4.7 nondeterminism.
